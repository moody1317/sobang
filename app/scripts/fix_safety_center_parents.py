...
def fix_safety_center_parents(filepath: str):
    ...
    with open(filepath, encoding="cp949") as f:
        ...
        for row in reader:
            ...
            if not center:
                unmatched += 1
                unmatched_list.append((center_name, station_name, "안전센터 자체를 못 찾음"))
                continue
            ...
            if not station:
                unmatched += 1
                unmatched_list.append((center_name, station_name, "소방서를 못 찾음"))
                continue
    ...
    print("매칭 안 된 목록:")
    for center_name, station_name, reason in unmatched_list:
